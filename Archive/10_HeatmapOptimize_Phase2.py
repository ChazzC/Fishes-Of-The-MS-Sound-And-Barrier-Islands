...
# Keep the complete hex_gdf in Python. Only send the fields that
# Leaflet needs to draw and identify a hexagon to the browser.
# This reduces the browser-side GeoJSON payload while preserving
# all environmental and species information for the selected hex.

HEX_MAP_FIELDS = ["id"]

hex_map_gdf = hex_gdf[HEX_MAP_FIELDS + ["geometry"]].copy()
...
# Lookup table containing the COMPLETE original hex records.
# Leaflet only sends the ID when a hex is clicked.
hex_lookup = {
    str(row["id"]): row
    ...
    if properties.get("_layer_type") == "hex":
        ...
        if active_hex_id != current_hex_id:
            ...
            if selected_row is not None:
                ...
                st.session_state.selected_hex = selected_row.to_dict()
