...
def build_species_search_options(metadata):
    options = []
    for scientific_name, record in metadata.items():
        scientific_name = str(scientific_name).strip()
        if not scientific_name:
            continue
        common_name = str(record.get("common_name") or "").strip() if isinstance(record, dict) else ""
        label = f"{scientific_name} — {common_name}" if common_name else scientific_name
        options.append((scientific_name, label))
    return sorted(options, key=lambda item: item[1].casefold())

def find_matching_hexes(hex_gdf, scientific_name):
    target = str(scientific_name).casefold()
    matching_ids = []
    for _, row in hex_gdf.iterrows():
        species = parse_species_array(row.get("Species_Array"))
        if any(str(name).casefold() == target for name in species):
            matching_ids.append(str(row["id"]))
    return matching_ids
...
if "map_search_species" not in st.session_state:
    st.session_state.map_search_species = None
...
species_search_options = build_species_search_options(species_metadata)
species_search_by_label = {label: scientific_name for scientific_name, label in species_search_options}
search_options = ["— Clear species highlight —"] + [label for _, label in species_search_options]

current_search_label = "— Clear species highlight —"
if st.session_state.map_search_species:
    for scientific_name, label in species_search_options:
        if scientific_name.casefold() == str(st.session_state.map_search_species).casefold():
            current_search_label = label
            break

selected_search_label = st.selectbox(
    "Find a fish species on the map",
    search_options,
    index=search_options.index(current_search_label),
    help="Search by scientific name or common name. Matching hexagons will be highlighted on the map.",
    key="species_map_search",
)

if selected_search_label == "— Clear species highlight —":
    st.session_state.map_search_species = None
else:
    st.session_state.map_search_species = species_search_by_label[selected_search_label]

matching_hex_ids = set()
if st.session_state.map_search_species:
    matching_hex_ids = set(find_matching_hexes(hex_gdf, st.session_state.map_search_species))
...
# ============================================================
# HIGHLIGHT HEXES FOR SPECIES SEARCH
# ============================================================

if matching_hex_ids:
    matching_features = []
    for feature in json.loads(hex_geojson)["features"]:
        feature_id = str(feature.get("properties", {}).get("id"))
        if feature_id in matching_hex_ids:
            feature = copy.deepcopy(feature)
            feature.setdefault("properties", {})
            feature["properties"]["_layer_type"] = "hex_species_match"
            matching_features.append(feature)

    if matching_features:
        matching_geojson = {"type": "FeatureCollection", "features": matching_features}
        folium.GeoJson(
            matching_geojson,
...
if st.session_state.map_search_species:
    search_record = find_species_metadata(st.session_state.map_search_species)
    if search_record and search_record.get("common_name"):
        search_name = f"{st.session_state.map_search_species}, {search_record['common_name']}"
    else:
        search_name = st.session_state.map_search_species
    st.caption(
        f"**{search_name}** occurs in **{len(matching_hex_ids):,} hexes**. "
        "Matching hexes are highlighted on the map."
    )
...
if active:
    ...
    if properties.get("_layer_type") in {"hex", "hex_species_match"}:
